...
def train_ensemble_classifier(X, y, grid_search=False, recursive_felim = False, weights = [1,1,1,1,1]):
    ...
    rfc = RandomForestClassifier(criterion='entropy', n_estimators=1000, max_depth=len(X.columns)/3)
...
# weights = lr, knn, nb, rfc, svc
clf = train_ensemble_classifier(X_train, y_train, weights = [1, 1, 0, 2, 1], grid_search=False, recursive_felim=True)#train_titanic_tree(X_train, y_train)
